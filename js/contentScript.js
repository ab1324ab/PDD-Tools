...
var cover_content = '<div style="position: fixed;top: 0px;opacity: 0.5;left: 0px;z-index: 1040;width: 100vw;height: 100vh;background-color: rgb(0, 0, 0);"></div>' +
    ...
    '<button style="position: absolute;right: 16px;top: 16px;" type="button" class="btn btn-outline-primary" onclick="document.getElementById(\'cover\').style.display =\'none\';clearInterval(timer);timer = null;">X</button>' +
    '</div>' +
    ...
    if (request.cmd == "open_back") {
        ...
        if (request.request.code == response_success) {
            ...
            timer = setInterval((function () {
                ...
                scroll.animate({scrollTop: 60 * (i+1)}, 1000);
